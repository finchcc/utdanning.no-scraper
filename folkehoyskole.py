...
def search_studies(session: requests.Session, start_num: int = 1, items_per_page: int = 8266):
    url = f"https://api.utdanning.no/search/result?page={start_num}&itemsPerPage={items_per_page}&hovedfasett=%22Utdanninger%22&utdanningsniva=%22Folkeh%C3%B8gskole%22"
...
def create_tables(conn: sqlite3.Connection):
    conn.execute('''CREATE TABLE IF NOT EXISTS folkehoyskole (
...
def main():
    ...
    create_tables(conn)
    session = new_session()
    search = search_studies(session, start_num=1)
    insert_studies(conn, search.member)
    # Write raw JSON search results to a text file
    with open('folkehoyskole_search_results.json', 'w', encoding='utf-8') as f:
